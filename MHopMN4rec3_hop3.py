...
class GRU4Rec:
    ...
    def __init__(self, loss, final_act, hidden_act, layers,
                 n_epochs=10, batch_size=50, dropout_p_hidden=0.5, dropout_p_embed=0.0, learning_rate=0.05, momentum=0.0, lmbd=0.0, embedding=0, Cate_embedding=0, n_sample=0, sample_alpha=0.75, smoothing=0,
                 adapt='adagrad', decay=0.9, grad_cap=0,
                 sigma=0, init_as_normal=False, reset_after_session=True, train_random_order=False, time_sort=True,
        ...
        self.MN_nfactors = 1#default:20
        self.MN_dims = embedding + Cate_embedding
    ...
    def change_embedding(self, embedding, Paths): # embedding: dict {name:vector}, Paths:Pandas DataFrame
        cateNum = max(embedding.keys()) + 1 + 1 #set the last as 0,0,0,...
        length = self.Cate_embedding#embedding[embedding.keys()[0]].shape[0]
        CateEmbedding = np.zeros((cateNum, length), dtype='float32')
        for i in embedding:
            CateEmbedding[i] = embedding[i]
        offset_C1, offset_C2, offset_C3 = -1 * np.zeros(self.n_items, dtype='int32'), -1 * np.zeros(self.n_items, dtype='int32'), -1 * np.zeros(self.n_items, dtype='int32')
        mergeE = np.zeros((self.n_items, length*3), dtype='float32')
        for i in Paths.values:
            if i[0] not in self.itemidmap:
                continue
            item = self.itemidmap[i[0]]
            mergeE[item] = np.concatenate((CateEmbedding[i[1]], CateEmbedding[i[2]], CateEmbedding[i[3]]), axis=0)
            #mergeE[item] = np.concatenate((CateEmbedding[i[1]], CateEmbedding[i[2]]), axis=0)
            offset_C1[item] = i[1]
            offset_C2[item] = i[2]
            offset_C3[item] = i[3]
        return CateEmbedding, offset_C1, offset_C2, offset_C3, mergeE #shape: n*d
    def init(self, data, ItemEmbedding, CateEmbedding, Paths):
        ...
        if self.Cate_embedding:
            CE, offset_C1, offset_C2, offset_C3, mergeE = self.change_embedding(CateEmbedding, Paths)
            self.CE = theano.shared(CE)
            self.mergeE = theano.shared(mergeE)
            self.offset_C1 = theano.shared(offset_C1)
            self.offset_C2 = theano.shared(offset_C2)
            self.offset_C3 = theano.shared(offset_C3)
        else:
        ...
        self.WY = self.init_weights((self.embedding+self.Cate_embedding*3, self.out_dim))
        self.BY = theano.shared(value=np.zeros((self.out_dim,), dtype=theano.config.floatX), borrow=True)

        ### add memory network
        ## parameters for read operator
        self.MN_hop1 = theano.shared(value=np.zeros((self.batch_size, self.MN_nfactors, self.MN_dims), dtype=theano.config.floatX), borrow=True)
        self.MN_hop2 = theano.shared(value=np.zeros((self.batch_size, self.MN_nfactors, self.MN_dims), dtype=theano.config.floatX), borrow=True)
        self.MN_hop3 = theano.shared(value=np.zeros((self.batch_size, self.MN_nfactors, self.MN_dims), dtype=theano.config.floatX), borrow=True)
        self.mask_vector = theano.shared(np.zeros((self.batch_size, 1), dtype=theano.config.floatX), borrow=True)
        self.MN_mask = theano.shared(value=-10000+np.zeros((self.batch_size, self.MN_nfactors), dtype=theano.config.floatX), borrow=True)
        self.MN_Wu_hop1 = self.init_weights((self.layers[-1], self.MN_dims))
        self.MN_Wu_hop2 = self.init_weights((self.MN_dims + self.MN_dims, self.MN_dims))
        self.MN_Wu_hop3 = self.init_weights((self.MN_dims + self.MN_dims, self.MN_dims))
        #self.MN_Bu = theano.shared(value=np.zeros((self.MN_dims,), dtype=theano.config.floatX), borrow=True)
        self.MN_Wu2 = self.init_weights(((3*self.MN_dims+self.layers[-1]), self.layers[-1]))
    ...
    def model(self, X, H, MN_hop1, MN_hop2, MN_hop3, MN_mask, Y=None, drop_p_hidden=0.0, drop_p_embed=0.0, predict=False):
        if self.embedding:
            Sx = self.E[X]
        ...
        ## write operator
        ItemE = self.E[X] #shape: b*d
        C1, C2, C3 = self.offset_C1[X], self.offset_C2[X], self.offset_C3[X]
        Cate1E, Cate2E, Cate3E = self.CE[C1], self.CE[C2], self.CE[C3] #shape: b*d
        IC1E, IC2E, IC3E = T.concatenate([ItemE, Cate1E], axis=-1), T.concatenate([ItemE, Cate2E], axis=-1), T.concatenate([ItemE, Cate3E], axis=-1) #shape: b*(2d)
        MN_hop1_temp, MN_hop2_temp, MN_hop3_temp = MN_hop1[:,1:,:], MN_hop2[:,1:,:], MN_hop3[:,1:,:] 
        MN_hop1 = T.concatenate([MN_hop1_temp, IC1E.dimshuffle(0,'x',1)], axis=-2) #shape: b*k*(2d)
        MN_hop2 = T.concatenate([MN_hop2_temp, IC2E.dimshuffle(0,'x',1)], axis=-2)
        MN_hop3 = T.concatenate([MN_hop3_temp, IC3E.dimshuffle(0,'x',1)], axis=-2)       
        MN_mask = MN_mask[:,1:] #shape: b*k        
        MN_mask = T.concatenate([MN_mask, self.mask_vector], axis=-1) #shape: b*k
        ## read operator
        U_trans = (self.hidden_activation(T.dot(y, self.MN_Wu_hop1))).dimshuffle(0, 1, 'x') # We need to make the same dimension. (shape:b*(2d)*1)
        MN_AW_hop1 = T.nnet.softmax(T.batched_dot(MN_hop1, U_trans)[:,:,0]+MN_mask) # shape:b*k
        MN_AC_hop1 = T.batched_dot(MN_hop1.dimshuffle(0,2,1), MN_AW_hop1.dimshuffle(0,1,'x'))[:,:,0] # shape: b*2d*1
        U_trans = T.concatenate([U_trans[:,:,0], MN_AC_hop1], axis=-1)
        U_trans = (self.hidden_activation(T.dot(U_trans, self.MN_Wu_hop2))).dimshuffle(0, 1, 'x')
        MN_AW_hop2 = T.nnet.softmax(T.batched_dot(MN_hop2, U_trans)[:,:,0]+MN_mask)# shape:b*k
        MN_AC_hop2 = T.batched_dot(MN_hop2.dimshuffle(0,2,1), MN_AW_hop2.dimshuffle(0,1,'x'))[:,:,0] # shape: b*2d*1
        U_trans = T.concatenate([U_trans[:,:,0], MN_AC_hop2], axis=-1)
        U_trans = (self.hidden_activation(T.dot(U_trans, self.MN_Wu_hop3))).dimshuffle(0, 1, 'x')
        MN_AW_hop3 = T.nnet.softmax(T.batched_dot(MN_hop3, U_trans)[:,:,0]+MN_mask)# shape:b*k
        MN_AC_hop3 = T.batched_dot(MN_hop3.dimshuffle(0,2,1), MN_AW_hop3.dimshuffle(0,1,'x'))[:,:,0] # shape: b*2d*1
        Ut = T.concatenate([y, MN_AC_hop1, MN_AC_hop2, MN_AC_hop3], axis=-1) # shape:b*(dim+2d)
        ...
        if Y is not None:
            Sy = T.concatenate([self.E[Y], self.mergeE[Y]], axis=-1) #shape:b*2d
            ...
            return H_new, MN_hop1, MN_hop2, MN_hop3, MN_mask, y, [Sx, Sy]
        else: ## output user embedding
            if predict == True:
                return H_new, MN_hop1, MN_hop2, MN_hop3, MN_mask, y, Sx
            else:
                return "error"
    ...
    def fit(self, data, ItemEmbedding, CateEmbedding, Paths, retrain=False, sample_store=10000000):
        '''
        Trains the network.
        ...
        if not retrain:
            ...
            offset_sessions = self.init(data, ItemEmbedding, CateEmbedding, Paths)
        else:
            print("We do not allow retrain, just normal train")
        X = T.ivector('X')
        #C1, C2, C3 = T.ivector('C1'), T.ivector('C2'), T.ivector('C3')
        Y = T.imatrix('Y')
        H_new, MN_hop1_new, MN_hop2_new, MN_hop3_new, MN_mask_new, Y_pred, sampled_params = self.model(X, self.H, self.MN_hop1, self.MN_hop2, self.MN_hop3, self.MN_mask, Y, self.dropout_p_hidden, self.dropout_p_embed)
        ...
        full_params = [self.Wy, self.By, self.WY, self.BY, self.MN_Wu_hop1, self.MN_Wu_hop2, self.MN_Wu_hop3, self.MN_Wu2, self.MN_Bu2]
        #full_params = [self.Wy, self.By, self.WY, self.BY, self.MN_Wu_hop1, self.MN_Wu_hop2, self.MN_Wu2, self.MN_Bu2]
        #full_params = [self.Wy, self.By, self.WY, self.BY, self.MN_Wu_hop1, self.MN_Wu2, self.MN_Bu2]
        sidxs = [X, Y]
        updates = self.RMSprop(cost, params, full_params, sampled_params, sidxs)
        for i in range(len(self.H)):
            updates[self.H[i]] = H_new[i]
        updates[self.MN_hop1] = MN_hop1_new
        updates[self.MN_hop2] = MN_hop2_new
        updates[self.MN_hop3] = MN_hop3_new
        updates[self.MN_mask] = MN_mask_new
        ...
        for epoch in range(self.n_epochs):
            ...
            self.MN_hop1.set_value(np.zeros((self.batch_size, self.MN_nfactors, self.MN_dims), dtype=theano.config.floatX), borrow=True)
            self.MN_hop2.set_value(np.zeros((self.batch_size, self.MN_nfactors, self.MN_dims), dtype=theano.config.floatX), borrow=True)
            self.MN_hop3.set_value(np.zeros((self.batch_size, self.MN_nfactors, self.MN_dims), dtype=theano.config.floatX), borrow=True)
            self.MN_mask = theano.shared(value=-10000+np.zeros((self.batch_size, self.MN_nfactors), dtype=theano.config.floatX), borrow=True)
            c = []
            ...
            while not finished:
                ...
                if len(mask) and self.reset_after_session:
                    ...
                    tmp = self.MN_hop1.get_value(borrow=True)
                    tmp[mask] = 0
                    self.MN_hop1.set_value(tmp, borrow=True)
                    tmp = self.MN_hop2.get_value(borrow=True)
                    tmp[mask] = 0
                    self.MN_hop2.set_value(tmp, borrow=True)
                    tmp = self.MN_hop3.get_value(borrow=True)
                    tmp[mask] = 0
                    self.MN_hop3.set_value(tmp, borrow=True)
                    tmp = self.MN_mask.get_value(borrow=True)
                    tmp[mask] = -10000
                    self.MN_mask.set_value(tmp, borrow=True)
            avgc = np.mean(c)
            if np.isnan(avgc):
    ...
    def predict_next_batch(self, session_ids, input_item_ids, predict_for_item_ids=None, batch=100):
        ...
        if self.predict is None or self.predict_batch!=batch:
            ...
            self.MN_hop1.set_value(np.zeros((batch, self.MN_nfactors, self.MN_dims), dtype=theano.config.floatX), borrow=True)
            self.MN_hop2.set_value(np.zeros((batch, self.MN_nfactors, self.MN_dims), dtype=theano.config.floatX), borrow=True)
            self.MN_hop3.set_value(np.zeros((batch, self.MN_nfactors, self.MN_dims), dtype=theano.config.floatX), borrow=True)
            self.mask_vector.set_value(np.zeros((batch, 1), dtype=theano.config.floatX), borrow=True)
            self.MN_mask = theano.shared(value=-10000+np.zeros((batch, self.MN_nfactors), dtype=theano.config.floatX), borrow=True)
            #if predict_for_item_ids is not None:
            #    H_new, yhat, _ = self.model(X, self.H, Y, predict=True)
            #else:
            H_new, MN_hop1_new, MN_hop2_new, MN_hop3_new, MN_mask_new, yhat, _ = self.model(X, self.H, self.MN_hop1, self.MN_hop2, self.MN_hop3, self.MN_mask, predict=True) # now, H_new save every layers' embedding, and yhat save the last layer out as user embedding
            
            updatesH = OrderedDict()
            for i in range(len(self.H)):
                updatesH[self.H[i]] = H_new[i]
            updatesH[self.MN_hop1] = MN_hop1_new
            updatesH[self.MN_hop2] = MN_hop2_new
            updatesH[self.MN_hop3] = MN_hop3_new
            updatesH[self.MN_mask] = MN_mask_new
            #if predict_for_item_ids is not None:
    ...
    def save_ItemEmbedding(self, data, filename = 'item_embedding'): #self.itemidmap, self.E
        itemids = data[self.item_key].unique()
        ME = T.concatenate([self.E, self.mergeE], axis=-1)
        MergeEmbedding = self.hidden_activation(T.dot(ME, self.WY) + self.BY)#.get_value(borrow=True)
